...
# create a function to compute the Total Within-Cluster Sum of Squares (WCSS) for a given k
get_wcss <- function(k) {
  kmeans_model <- kmeans(df_clean %>% select(longitude, latitude), centers = k, nstart = 25)
...
# plot the elbow curve plot. commented this out because we don't want it to print for our EDA
...
kmeans_result <- kmeans(df_clean %>% select(longitude, latitude), centers = 3, nstart = 25)
...
ggplot() +
  ...
  geom_point(data = df_clean, aes(x = longitude, y = latitude, color = cluster), 
             alpha = 0.6, size = 1) +
  coord_quickmap(xlim = c(40, 200), ylim = c(-90, 0)) + # zooming map in to just australia-ish area
  theme_minimal() +
